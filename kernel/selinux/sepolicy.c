...
static bool add_type(struct policydb *db, const char *type_name, bool attr)
{
   ...
   */
    ...
    if (!new_type_attr_map || !new_type_val_to_struct || !new_val_to_name_types) {
        pr_err("add_type: alloc failed for Mediatek\n");
        kfree(new_type_attr_map);
        kfree(new_type_val_to_struct);
        kfree(new_val_to_name_types);
        return false;
    }
